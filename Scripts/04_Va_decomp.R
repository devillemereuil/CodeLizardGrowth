...
## Newborn model (L0)
# Loading the model
mod_L0 <- readRDS(here("Output/animal_model_L0_1sex.rds"))
...
vr_nl <-
    ...
    as.vector()
# Trimming for compatibility with L0
tbl_est_NL <-
    tbl_est_NL |>
    filter(Iter %% 2 == 0) |>
    mutate(Iter = Iter / 2)
...
if (run_decomp) {
    ...
    saveRDS(tbl_vars, here("Output/Variances_Age_nonlinear.rds"))
} else {
    # Merely load the results
    tbl_vars <- readRDS(here("Output/Variances_Age_nonlinear.rds"))
}
...
if (run_decomp) {
    ...
    saveRDS(var_decomp_all_F, file = here("Output/var_decomp_all_F.rds"))
    ...
    saveRDS(var_decomp_all_M, file = here("Output/var_decomp_all_M.rds"))
    ...
    write_ods(var_decomp_all_F |>
              ...
              summarise(Mean = mean(Value) |> signif(digits = 3),
                        ...
                        .by = Parameter),
              path = here("Tables/VA_decomp.ods"),
              sheet = "Females (all)")
    ...
    # Merely load the results
    var_decomp_all_F <- readRDS(here("Output/var_decomp_all_F.rds"))
    var_decomp_all_M <- readRDS(here("Output/var_decomp_all_M.rds"))
}
...
if (run_decomp) {
    ...
    saveRDS(var_decomp_rec_F, file = here("Output/var_decomp_rec_F.rds"))
    ...
    saveRDS(var_decomp_rec_M, file = here("Output/var_decomp_rec_M.rds"))
    ...
    # Merely load the results
    var_decomp_rec_F <- readRDS(here("Output/var_decomp_rec_F.rds"))
    var_decomp_rec_M <- readRDS(here("Output/var_decomp_rec_M.rds"))
}
...
write_file(latex_smry, file = here("Tables/Estimates_reacnorm.tex"))
...
cairo_pdf(here("Figures/Genetic_variance_Age.pdf"), width = 8, height = 6)
plot(p_vars)
dev.off()
...
cairo_pdf(here("Figures/Herit_Age.pdf"), width = 8, height = 6)
plot(p_herit)
dev.off()
...
cairo_pdf(here("Figures/V_A_h2_Age.pdf"), width = 6, height = 8)
plot(p_both)
dev.off()
...
cairo_pdf(here("Figures/Gamma_Age.pdf"), width = 7, height = 6)
plot(p_gamma)
dev.off()

## Full graph for the decomposition

cairo_pdf(here("Figures/Full_decomp.pdf"), width = 10, height = 6)
